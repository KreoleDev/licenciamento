...
import {createOrUpdateClass} from '@/app/[locale]/(myapp)/functions/classes'

export default function Formmodalclass({ openModal, setOpen, initialData } : { openModal: boolean, setOpen: () => void, initialData?: any }) {
...
async function handleCreateUpdateClass (values: z.infer<any>): Promise<void  | undefined> {
  ...
  try {
  await createOrUpdateClass({ ...initialData, ...values });
  igrpToast({
    type: "success",
    title: "Sucsess",
...
formRef={ formform1Ref }
  onSubmit={ handleCreateUpdateClass }
